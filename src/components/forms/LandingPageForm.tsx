...
const LandingPageForm = ({ title, bold }: { title: string; bold: string }) => {
      ...
      content: (
        <div className="flex gap-2 text-base">
          <select
            ...
            style={{ inlineSize: `${countryCode.length + 2.5}ch` }}
          ...
          <input
            ...
            className="w-full bg-transparent rounded-md placeholder:text-primary text-black no-spinner focus:outline-none"
          />
        </div>
      ),
    },
  ...
  return (
    <form
      ...
      className="flex flex-col gap-4 max-md:px-4 p-6 max-md:mt-6 text-base rounded-lg lg:max-w-[24.875rem] w-full bg-primary2 text-white"
    >
      ...
      {formData.map((data, index) => (
        <div key={index} className="flex flex-col gap-1">
          <div className="flex  gap-2 text-secondary p-3 border bg-white border-blue-primary rounded-md">
            <label
              htmlFor={data.name}
              className={`${data.tag === "textarea" && "mt-1"} text-primary2`}
            >
              {data.icon}
            </label>
            {data.tag === "div"
