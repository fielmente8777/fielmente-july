      ...
      <Container className="md:py-[7.5rem] relative after:absolute after:inset-0 after:bg-[url('/map.png')] after:bg-contain after:bg-no-repeat after:opacity-8 after:bg-center after:w-full after:h-full after:z-[-1]">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-14">
          <div className="flex flex-col gap-6">
            ...
            <p className="text-white text-xl">{footerData.description}</p>
              ...
              <ul className="flex flex-col gap-5">
                {item.listOfLinks.map((link, index) => (
                  <li key={index}>
                    <Link
                      href={link.href}
                      className="text-white hover:text-secondary transition-all duration-300 ease-in-out  text-xl"
                    >
                      {link.label}
                    </Link>
                  </li>
                ...
                <ul className="flex flex-col gap-6">
                  {item.listOfLinks.map((link, index) => (
                    <li key={index}>
                      <Link
                        href={link.href}
                        className="text-white hover:text-secondary flex gap-2 transition-all duration-300 ease-in-out  text-xl"
                      >
